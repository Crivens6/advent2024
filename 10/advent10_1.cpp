...
set<pair<int, int>> FindTrails(int row, int col, int target_elevation)
{
    ...
    for (int dir = Direction::N; dir <= Direction::W; dir++)
    {
            ...
            0 <= next_col && next_col < g_map_grid[0].size())
        {
            ...
            {
                destinations.merge(FindTrails(next_row, next_col, target_elevation + 1));
            }
        }
    }
    return destinations;
...
int CheckTrailheadScore(int row, int col)
{
    ...
    {
        return FindTrails(row, col, kTrailHead + 1).size();
    }
