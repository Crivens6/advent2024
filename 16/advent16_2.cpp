...
private:
    ...
    set<pair<int, int>> connection_area[3];
    int distance;
    set<pair<int, int>> min_area;

public:
    SimpleNode(int node_a_index, int node_a_dist, set<pair<int, int>> area_a, int node_b_index, int node_b_dist, set<pair<int, int>> area_b, int node_c_index, bool is_end = false)
    {
        connected_nodes[0] = node_a_index;
        connection_score[0] = node_a_dist;
        connection_area[0] = area_a;
        connected_nodes[1] = node_b_index;
        connection_score[1] = node_b_dist;
        connection_area[1] = area_b;
    ...
    void PrintNodePath()
    {
        for (int x = 0; x < g_map_grid.size(); x++)
        {
            for (int y = 0; y < g_map_grid[0].size(); y++)
            {
                if (min_area.find({x, y}) != min_area.end())
                {
                    std::cout << "O";
                }
                else
                {
                    if (g_map_grid[x][y] == MazeSquare::Wall)
                    {
    ...
    set<pair<int, int>> GetMinArea()
    {
        return min_area;
    }
    void SetMinDistance(int dist, set<pair<int, int>> path_area)
    {
        if (distance == -1)
        {
            min_area = path_area;
        ...
        else if (distance >= dist)
        {
            if (distance == dist)
            {
                min_area.insert(path_area.begin(), path_area.end());
            }
            else
            {
                min_area = path_area;
    ...
    set<pair<int, int>> GetNeighborPathArea(int index)
    {
        return connection_area[index];
    }
};
...
private:
    ...
    set<pair<int, int>> connection_area[4];
...
public:
    ...
    MazeNode(int index, Direction node_dir, int other_node, int path_score, Direction other_node_return_dir, set<pair<int, int>> area)
    {
        connected_nodes[0] = -1;
        connected_nodes[1] = -1;
        connected_nodes[2] = -1;
        ...
        connection_area[node_dir] = area;
    }

    void CreateConnection(Direction node_dir, int other_node, int path_score, Direction other_node_return_dir, set<pair<int, int>> area)
    {
        connection_score[node_dir] = path_score;
        connected_nodes[node_dir] = other_node;
        connected_return_direction[node_dir] = other_node_return_dir;
    ...
    {
        SimpleNode horizontal = SimpleNode(SimpleNodeIndex(1), connection_score[1], connection_area[1], SimpleNodeIndex(3), connection_score[3], connection_area[3], (self_index * 2) + 1);
        SimpleNode vertical = SimpleNode(SimpleNodeIndex(0), connection_score[0], connection_area[0], SimpleNodeIndex(2), connection_score[2], connection_area[2], (self_index * 2));
        return {horizontal, vertical};
    }
    vector<SimpleNode> SplitEndNode()
    {
        SimpleNode horizontal = SimpleNode(SimpleNodeIndex(1), connection_score[1], connection_area[1], SimpleNodeIndex(3), connection_score[3], connection_area[3], (self_index * 2) + 1, true);
        SimpleNode vertical = SimpleNode(SimpleNodeIndex(0), connection_score[0], connection_area[0], SimpleNodeIndex(2), connection_score[2], connection_area[2], (self_index * 2), true);
        return {horizontal, vertical};
    }
...
private:
    ...
    int path_score;
    int parent_node;
    set<pair<int, int>> covered_area;
    void SetSpot(pair<int, int> coord)
    {
        x = coord.first;
        y = coord.second;
        covered_area.insert(coord);
    }
...
public:
    PathFinder(pair<int, int> coords, int parent_node, Direction dir, int starting_distance = 0)
    {
        ...
        this->covered_area.insert(GetNextCoord((Direction)(((int)dir + 2) % 2)));
        this->covered_area.insert(coords);
    }
    ...
    deque<PathFinder> Explore()
    {
        ...
        while (!path_explored)
        {
            ...
            if (g_node_map.find({x, y}) != g_node_map.end())
            {
                ...
                g_node_list[g_node_map[{x, y}]].CreateConnection(return_dir, parent_node, path_score, departed_dir, covered_area);

                // Give node information to parent node
                g_node_list[parent_node].CreateConnection(departed_dir, g_node_map[{x, y}], path_score, return_dir, covered_area);
            ...
            if (possible_steps > 1)
            {
                ...
                MazeNode new_node(new_node_index, return_dir, parent_node, path_score, departed_dir, covered_area);
                g_node_list.push_back(new_node);
                g_node_map[{x, y}] = new_node_index;

                // Give node information to parent node
                g_node_list[parent_node].CreateConnection(departed_dir, new_node_index, path_score, return_dir, covered_area);
...
int main()
{
    ...
    while (visited_nodes.find(end_node) == visited_nodes.end())
    {
        ...
        set<pair<int, int>> min_area = simple_nodes[min_index].GetMinArea();
        // Visit Neighbors
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                set<pair<int, int>> area_to_neighbor = min_area;
                set<pair<int, int>> just_to_neighbor = simple_nodes[min_index].GetNeighborPathArea(i);
                area_to_neighbor.insert(just_to_neighbor.begin(), just_to_neighbor.end());
                simple_nodes[neighbor.first].SetMinDistance(min_distance + neighbor.second, area_to_neighbor);
            }
        }
    }
